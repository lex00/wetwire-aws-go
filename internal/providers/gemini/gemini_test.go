...
func TestProvider_Name(t *testing.T) {
	...
	if skipTest() {
		t.Skip("Skipping Gemini test (GEMINI_API_KEY not set)")
	}
...
func testingEnabled() bool {
	// Tests run if GEMINI_API_KEY is set or CI env var is set
	// In CI, we skip unless explicitly enabled with RUN_GEMINI_TESTS=1
	apiKey := getAPIKey()
	if apiKey != "" {
		return true
	}
